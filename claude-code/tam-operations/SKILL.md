...
name: tam-operations
title: "TAM Operations"
category: custom
version: "1.1.0"
updated: "2026-06-01"
tags: [tam, account-management, customer-success, case-ops, incident-response, reporting, ts-tools, hub]
keywords:
  - TAM account deliverables EBR QBR account review support plan JIMP
  - account health scoring churn risk NRR customer sentiment
  - MongoDB Premium Services TAM operating model roles incident SLA
  - account artifact collection report generation
  - operator report shift handoff SBAR meeting prep BLUF
  - customer file consolidation TAM briefing
  - active case tracker TS Tools Support API severity diagnostic catalog
  - incident response SEV postmortem SLO error budget on-call
  - autoremediation retry circuit breaker self-healing
  - firedrill game-day integration testing
  - TS Tools Support API endpoints auth Socket.IO
whenToUse:
  - "write or review a TAM deliverable (EBR, QBR, account review, support plan, JIMP, weekly update, case analysis)"
  - "assess account health / churn risk / NRR or design a health-scoring algorithm"
  - "look up the MongoDB Premium Services TAM operating model (roles, incident SLAs, JIMP, Straight-to-8, Monday tracking)"
  - "collect account artifacts across MCP sources, or consolidate a customer's local files into a briefing"
  - "build shift-handoff / meeting-prep / template-driven operator reports (SBAR, BLUF, freshness scoring)"
  - "build or extend the active case tracker, severity model, or diagnostic tool catalog"
  - "render a case/event timeline visualization in vanilla JS"
  - "design incident response (SEV, IC roles, postmortems, SLO/error budgets, on-call) or self-healing autoremediation"
  - "run firedrill/game-day validation, or implement/debug TS Tools Support API calls and endpoint shapes"
  - "run an autonomous Monday.com board audit (load references/monday-board-audit.md)"
whenNotToUse:
  - "MongoDB data-plane/engine, Atlas platform, or live cluster diagnostics (use mongodb-expert / mongodb-atlas-expert / atlas-diagnostics-expert)"
  - "MongoDB KB-article / error-code lookup (use mongodb-kb)"
  - "starting/using the local case MCP server and choosing mdb_case_* tools (use case-mcp-server-guide)"
  - "the prose craft of a deliverable (use writing-expert / technical-writing-craft / executive-comms)"
  - "metric modeling, funnels, cohorts, forecasting (use the da-* hubs)"
  - "templatizing an existing report into a reconstruction prompt (use content-ingestion-extraction)"
related_skills:
  - misc-catch-all
  - writing-expert
