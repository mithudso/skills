...
whenToUse: >-
  Use when reasoning about agreement, consistency, replication, or fault tolerance across multiple
  nodes — picking or explaining a consensus algorithm (Paxos/Raft/PBFT/HotStuff/Tendermint),
  reading or grounding claims about CAP/FLP/linearizability, understanding why a system needs 2f+1 or
  3f+1 replicas, or explaining how a blockchain reaches consensus (PoW longest-chain, PoS, Ethereum's
  Gasper, fork-choice rules, finality, Sybil resistance, the trilemma, consensus-layer attacks). Covers
  the consensus MECHANISM, not whole-chain designs or cryptographic primitive math.
keywords:
  - consensus
  - distributed systems
  - CAP theorem
