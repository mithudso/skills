...
- Before `tm_dev.md`: read `skeletons/skeleton-tm_dev.md`
...
34. **NEVER delete, modify, or remove any existing `threat-model-*` or `threat-model-compare-*` folders** in the repository. Only write to your own output folder (named from the ticket ID or timestamp per `output-formats.md`). Cleaning up temporary git worktrees you created is allowed; deleting other report folders is FORBIDDEN.
...
**Rule 4 — Output folder path.** The parent creates the output folder (ticket-ID-based or timestamped, per `output-formats.md`) in Step 1 and uses that exact path for ALL `create_file` calls. If a sub-agent needs to read previously written report files, pass the folder path in the sub-agent prompt.
...
1. **Record start time & gather context**
   ...
   **⛔ SOURCE TYPE DETECTION — Determine input source and populate `source_documents`:**
   The analysis source may be a git repository OR one or more documents fetched via an external tool (Glean, web, etc.). Detect which applies and record accordingly in `threat-inventory.json`:

   - **Document-based analysis** (user provided a URL, Glean document, Google Doc, Confluence page, etc.):
     - For each document fetched, record in `source_documents`:
       - `url` — the full URL provided by the user or returned by Glean
       - `title` — the document title as returned by Glean or extracted from the document
       - `type` — infer from context: technical design specs → `tech_design`; product/feature requirements → `product_description`; scope or charter documents → `scope`; anything else → `other`
       - `retrieval_tool` — `glean` if fetched via Glean MCP, `web` if fetched directly via URL
       - `retrieved_at` — use `START_TIME`
     - Set `commit`, `branch`, and `repository` to `"N/A — document analysis"` in the inventory

   - **Repository-based analysis** (user pointed to a codebase):
     - Set `source_documents: []`
     - Populate `commit`, `branch`, and `repository` from git commands above
...
8c. **Write developer brief** (`tm_dev.md`)
   - **Read `skeletons/skeleton-tm_dev.md` first** — copy skeleton structure, fill `[FILL]` placeholders
   - Filter `3-findings.md` to design-confident items only — see inclusion/exclusion criteria in `output-formats.md`
   - Rewrite each item in plain English: no CVSS, CWE, OWASP, STRIDE labels, tier labels, or SDL severity labels
   - "Impact on this system" must name specific components, data, or users from THIS system
   - Sort High → Medium → Low, then alphabetically by component within each priority band
   - **Reference:** `output-formats.md` for filtering rules, priority mapping, and self-check
...
8b. **Generate threat inventory** (`threat-inventory.json`)
   ...
   - Include git metadata (commit SHA, branch, date), `source_documents` array (populated in Step 1), and analysis metadata (model, timestamps)
