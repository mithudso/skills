...
Create a named folder at the start of analysis using the ticket ID from the user's prompt:
- Format: `threat-model-{TICKET-ID}` where `TICKET-ID` is the Jira ticket number (e.g., `CLOUDP-353786`, `SERVER-1234`)
- Examples: `threat-model-CLOUDP-353786`, `threat-model-SERVER-1234`
- If no ticket ID is provided in the prompt, fall back to a timestamped folder: `threat-model-YYYYMMDD-HHmmss` (UTC time)
...
| `tm_dev.md` | Developer-focused design issues brief (no security jargon) | Yes |
...
**Purpose:** Developer-friendly summary of design-level security considerations. No security taxonomy, no implementation details. Intended to inform design decisions before code is written.

> **⛔ IMPORTANT: Before writing this file, read [skeleton-tm_dev.md](./skeletons/skeleton-tm_dev.md) and copy the skeleton VERBATIM. Fill in the `[FILL]` placeholders.**

### Inclusion Criteria — What to Include

Only include findings that meet ALL of these criteria:
1. **Design-level**: The issue is an architectural or design decision the team can address before writing code — not a code-level implementation detail
2. **Confident**: There is clear evidence in the current architecture — not speculative, not "if the code does X"
3. **Actionable now**: A developer can make a concrete design choice about it today

**Include these issue types:**
- Missing authentication/authorization controls at trust boundaries
- Missing rate limiting on user-facing or public APIs
- Potential SSRF: any flow where the system fetches URLs or resources derived from external input
- Insufficient access control design (who can call what, across which boundaries)
- Missing audit logging for sensitive operations at design level
- Unencrypted data flows between components (e.g., no TLS planned between services)
- Overly permissive trust boundary design
- Missing CSRF protection for state-changing operations exposed externally

**Exclude these issue types:**
- Input sanitization and validation (implementation detail — no code yet)
- SQL injection, XSS, buffer overflow, command injection (code-level)
- Old or weak certificate/TLS versions (operational concern, not design)
- Library or dependency version vulnerabilities (dependency management)
- Anything listed under "Needs Verification" in `0-assessment.md`
- Tier 3 findings (require prior system compromise — not pre-implementation design decisions)

### Priority Mapping

| Finding Severity | tm_dev.md Priority |
|------------------|--------------------|
| Critical | High |
| Important | High |
| Moderate | Medium |
...
1. Read `3-findings.md` and filter to qualifying items using the inclusion/exclusion criteria above
2. Derive each `tm_dev.md` entry from the finding — do NOT copy finding language verbatim
3. Rewrite Risk, Impact, and Design consideration in plain English with no CVSS scores, CWE IDs, OWASP tags, tier labels, or STRIDE terminology
4. "Impact on this system" MUST name actual components, data, or users from THIS system — not a generic risk statement
5. If no findings pass the inclusion criteria, include the file with: `*No design-level issues requiring attention before implementation were identified.*`
6. Sort by priority (High first), then alphabetically by component name within each priority

### Self-Check

After writing `tm_dev.md`:
- [ ] No CVSS scores, CWE references, OWASP categories, STRIDE labels, or tier labels anywhere in the file
- [ ] No SDL Bugbar Severity labels (Critical / Important / Moderate / Low) — use only High / Medium / Low
- [ ] Every "Impact on this system" names a specific component, dataset, or user group from THIS system
- [ ] No input sanitization, SQL injection, XSS, or certificate version issues included
- [ ] File begins with `# Developer Security Brief` on line 1 (no code fence wrapper)
...
| [tm_dev.md](tm_dev.md) | Developer-focused design issues brief |
```
...
- `"baseline_commit": "2dd84ab"` — the commit SHA of the baseline report (`"N/A — document analysis"` for doc-to-code transition)
- `"target_commit": "abc1234"` — the commit being analyzed
- `"schema_version": "1.1"` — incremental repo-to-repo reports use schema version 1.1; doc-to-code transition reports use `"1.2"`
- `"analysis_mode": "incremental"` — set to `"incremental"` for repo-to-repo; set to `"transition"` for doc-to-code
...
- `"implementation_review"` — *(transition mode only)* design-doc component matched to code implementation
- `"not_implemented"` — *(transition mode only)* design-doc component with no corresponding code found
