...
- **URL:** run `DISTILL fetch --url <U>` to get plain text (or use `firecrawl`/`exa`/
  `WebFetch` if you need JS-rendered content — firecrawl already strips boilerplate).
...
- `DISTILL render --in units.json` → writes `~/.claude/distillations/<slug>-<date>.{md,json}`
