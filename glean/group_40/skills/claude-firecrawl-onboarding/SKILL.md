...
description: |
  Firecrawl ONBOARDING + install router. Use this to set Firecrawl up: run the one-command
  install (CLI + build + workflow skills + browser auth), get or authorize an API key
  (dashboard/CLI, or WorkOS ID-JAG on supported platforms), verify the install, and route to the
  right usage path — A: live web tools · B: integrate into app code · C: repeatable deliverables ·
  D: account/API-key auth · E: REST API with no install.
  TRIGGER: "install firecrawl", "set up firecrawl", "get/authorize a firecrawl API key",
  "firecrawl onboarding", "which firecrawl path or skill do I use", "firecrawl 401 / credentials".
  SKIP: actually searching / scraping / interacting with the web right now → firecrawl (CLI),
  firecrawl-search, firecrawl-scrape, firecrawl-interact, firecrawl-crawl, firecrawl-map;
  integrating Firecrawl into product code → firecrawl-build-onboarding / -scrape / -search /
  -interact / -parse; producing a finished deliverable → firecrawl-workflows.
---
