...
**Original Path:** claude-code/document-critique

## Description
Multipass document review agent that surfaces and fixes findings through passes 0–14 plus sub-passes 10.5 and 11.5 (intent through human-voice rephrasing, incl. authoritative verification and adversarial/hallucination guard) with a convergence loop until no medium-or-higher findings remain. Use when reviewing, critiquing, auditing, or iteratively refining any document — spec, RFC, runbook, playbook, KB, README, weekly update, customer-facing summary, or training doc. TRIGGER: "review this doc", "critique this spec", "is this good enough to ship", "fact-check", "clean up generator artifacts", "second pair of eyes", "keep going until no findings remain", "make this sound human", "audit this KB article", "validate this runbook". SKIP: writing or drafting a new document from scratch (use writing-expert); review target is source code not prose (use software-engineering-patterns); document is a pure machine-readable artifact (YAML/JSON/CSV) with no prose claims; document is under ~10 lines.
...
These tiers and exit conditions are this skill's calibration of the canonical model in `~/.Codex/skill-consolidation/convergence-and-severity.md` (shared with prompt-deep-optimizer, skill-optimizer, and ddo); keep them consistent with it.
...
If the document embeds runnable prompt artifacts (system-prompt blocks, prompt templates), audit them by dispatching prompt-deep-optimizer's relevant pass bundle as a bounded subagent and merge its findings into this critique's findings table under this skill's severity calibration — never a second nested loop (see "Composed artifacts" in `~/.Codex/skill-consolidation/convergence-and-severity.md`).
...
2. Apply every **blocking**, **major**, and **medium** fix — on iteration 1, before applying any fix, snapshot the target to the central backup directory per the pre-write snapshot guardrail in `~/.Codex/skill-consolidation/convergence-and-severity.md` (no-write modes such as the ddo driver's `--read-only`/`--annotate` need no snapshot)
...
6. Terminate or continue? Run `~/.Codex/skill-consolidation/convergence_check.py` on the previous and current document versions (passing the model-counted severity totals) to compute the stable-rewrite and no-progress verdicts — never estimate edit distance yourself.

**Terminate when any one is true:**
- Any of exit conditions 1–6 of `~/.Codex/skill-consolidation/convergence-and-severity.md` fires (clean / no-progress / content-cycling / stable-rewrite / loop-instability / iteration cap) — Read that file once at loop start rather than re-deriving the conditions. Doc-specific calibration: iteration cap **3**, raised to 5 only if Med+ findings dropped ≥50% in the prior iteration.
...
At the end of the run, append one telemetry row per executed pass per the canonical Telemetry schema in `~/.Codex/skill-consolidation/convergence-and-severity.md` (fail-safe — a write error never blocks the run).
