...
keywords:
  ...
  - document store knowledge base bootstrap taxonomy
  - DOM scraping content script MutationObserver shadow DOM SPA
  - Granola meeting transcript API ProseMirror sync
  - Plaud recorder transcription API MCP
  - document deconstruction reconstruction prompt data-manifest templatize
  - content ingestion extraction pipeline
whenToUse:
  - "audit an aging doc/runbook/KB for staleness, dead links, or drift and decide its fate"
  - "organize or bootstrap a document folder / Drive engagement / KB to a numbered taxonomy"
  - "build or debug a resilient Chrome content-script DOM extractor (selectors, MutationObserver, shadow DOM, SPA)"
  - "fetch Granola meeting transcripts/notes via API, native host, or a polling sync loop"
  - "integrate Plaud recorder transcripts via the Developer Platform API or MCP"
  - "reverse-engineer/templatize a document into a reconstruction prompt + data-manifest"
whenNotToUse:
  - "produce or parse a finished file format — PDF/Word/Excel/PowerPoint/CSV/JSON/draw.io (use document-formats)"
  - "general Chrome-extension architecture/storage that is not DOM extraction (use chrome-extension-expert)"
  - "prose/structure/voice critique of a document (use document-critique or writing-expert)"
  - "generate a report from scratch (use operator-report-generator under tam-expertise)"
  - "Plaud/Granola MCP tool-call usage details (use using-plaud-mcp)"
related_skills:
  - document-formats
