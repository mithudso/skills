...
whenToUse: Use when the task involves the Splunk platform (Splunk Enterprise / Splunk Cloud Platform) or its Search Processing Language — designing or explaining Splunk architecture and data lifecycle, writing or optimizing SPL searches, building knowledge objects/dashboards/alerts, working with Splunk Enterprise Security (SIEM/RBA), or placing Splunk Observability Cloud and the OpenTelemetry Collector relative to the log/event platform. Not for shipping logs into Splunk from MongoDB, generic OTel-SDK instrumentation, telemetry-routing pipelines, or Elasticsearch.
keywords:
  - Splunk
  - SPL
