...
**Original Path:** claude-code/firecrawl-onboarding
...
`~/.Codex/.env` (kept out of this skill file so it is not synced to the
hub registry or shared). Load it into your environment before running
Firecrawl:

```bash
set -a; . ~/.Codex/.env; set +a   # or add FIRECRAWL_API_KEY to your shell profile / a project .env
```

```dotenv
# ~/.Codex/.env  (chmod 600 — do not commit)
FIRECRAWL_API_KEY=fc-...
```