...
- [ ] File begins with `# Developer Security Brief` on line 1 (no code fence)
- [ ] No CVSS scores: search for `CVSS` — must return 0 matches
- [ ] No CWE references: search for `CWE-` — must return 0 matches
- [ ] No OWASP references: search for `OWASP` or `A0\d:20` — must return 0 matches
- [ ] No STRIDE labels: search for `Spoofing`, `Tampering`, `Repudiation`, `Denial of Service`, `Elevation of Privilege` — must return 0 matches
- [ ] No tier labels: search for `Tier 1`, `Tier 2`, `Tier 3` — must return 0 matches
- [ ] No SDL Bugbar severity labels: search for `| Critical |`, `| Important |` as priority values — must return 0 matches (only `High`, `Medium`, `Low` allowed)
- [ ] No excluded issue types: search for `input saniti`, `SQL injection`, `XSS`, `buffer overflow`, `certificate version`, `TLS version` — must return 0 matches
- [ ] Every "Impact on this system" section contains a specific component, dataset, or user group name — not a generic statement like "attackers could compromise the system"
- [ ] Summary table present with columns: `# | Issue | Affected Component | Priority`
- [ ] Each issue has exactly 3 sub-fields: `**Risk:**`, `**Impact on this system:**`, `**Design consideration:**`
...
- [ ] **Report Files completeness**: Every generated `.md` and `.mmd` file in the output folder has a corresponding row in the Report Files table (`threat-inventory.json` is intentionally excluded). This includes `tm_dev.md`.
...
- [ ] **Simplified display tags only**: Search ALL `.md` files for old-style tags: `[STILL PRESENT]`, `[NEW CODE]`, `[NEW IN MODIFIED]`, `[PREVIOUSLY UNIDENTIFIED]`, `[PARTIALLY MITIGATED]`, `[REMOVED WITH COMPONENT]`, `[MODIFIED]`. ❌ Any match → FAIL. Replace with simplified tags: `[Existing]`, `[Fixed]`, `[Partial]`, `[New]`, `[Removed]` (standard mode) or `[Confirmed]`, `[Fixed]`, `[New]`, `[Not Implemented]` (transition mode).
- [ ] **Valid display tags**: In **standard mode**, every finding/threat annotation uses exactly one of: `[Existing]`, `[Fixed]`, `[Partial]`, `[New]`, `[Removed]`. In **transition mode** (`"analysis_mode": "transition"`), every finding/threat annotation uses exactly one of: `[Confirmed]`, `[Fixed]`, `[New]`, `[Not Implemented]`. Tags must appear as blockquote on first line of body: `> **[Tag]**`.
- [ ] **Component status simplified**: In standard mode, component status column uses only: `Unchanged`, `Modified`, `New`, `Removed`. ❌ `Restructured` → FAIL (use `Modified` instead). In transition mode, also allow: `Implementation Review`, `Not Implemented`.
- [ ] **Change Summary tables use simplified tags**: In standard mode, Threat Status table has 4 rows (Existing/Fixed/New/Removed) and Finding Status table has 5 rows (Existing/Fixed/Partial/New/Removed). In transition mode, both tables use: Confirmed/Fixed/New/Not Implemented. ❌ Old-style rows like `Still Present`, `New (Code)`, `Partially Mitigated` → FAIL in either mode.
...
- [ ] **No security taxonomy**: zero occurrences of `CVSS`, `CWE-`, `OWASP`, `Tier 1/2/3`, `SDL Bugbar`, `Spoofing`, `Tampering`, `Repudiation`, `Denial of Service`, `Elevation of Privilege`
- [ ] **Priority values**: only `High`, `Medium`, `Low` used — NOT `Critical`, `Important`, `Moderate`
- [ ] **No excluded issue types**: `input saniti`, `SQL injection`, `XSS`, `buffer overflow`, `certificate version` do not appear
- [ ] **Impact specificity**: every "Impact on this system" section names a component, dataset, or user group from this specific system
- [ ] **Summary table present** with columns `# | Issue | Affected Component | Priority`
- [ ] **Each issue has exactly 3 fields**: Risk, Impact on this system, Design consideration
- [ ] **No code fence on line 1**: file starts with `# Developer Security Brief`
...
- [ ] **`schema_version` field** — Present and equals `"1.0"` (standalone), `"1.1"` (incremental repo-to-repo), or `"1.2"` (doc-to-code transition). If the report contains `"incremental": true`, schema_version MUST be `"1.1"` or `"1.2"`. If `"analysis_mode"` is `"transition"`, schema_version MUST be `"1.2"`. Otherwise `"1.0"`.
- [ ] **`analysis_mode` field** — Present and equals `"standalone"`, `"incremental"`, or `"transition"`. Must be consistent with `schema_version`: `standalone` → `"1.0"`, `incremental` → `"1.1"`, `transition` → `"1.2"`.
