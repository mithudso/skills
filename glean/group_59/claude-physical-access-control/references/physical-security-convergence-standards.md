...
> **Reference file — part of the `physical-access-control` hub.** Formerly the standalone `physical-security-convergence-standards` skill.
...
---
name: physical-security-convergence-standards
description: >-
  ...
  (IdP/directory/PIAM, one credential for door + network, CSO/CISO convergence);
  US federal identity for physical access (HSPD-12, FIPS 201/PIV, PIV-I, CAC,
  FICAM, OMB M-19-17, SP 800-116 facility-area to auth-factor model, APL, Federal
  PKI/OCSP, SP 800-73/800-78 by name); standards bodies & frameworks (SIA; ONVIF
  Profiles A/C/D; ISO/IEC 27001 Annex A.7; SOC 2 CC6; PCI DSS req 9; ISO/IEC
  60839 naming); PSIM / command center; visitor management & identity proofing;
  biometric-privacy law (Illinois BIPA) + surveillance-notice (jurisdiction- and
  date-dependent, not advice). SKIP: RF bands/physics ->
  rfid-fundamentals-bands-standards; NDEF/NFC -> nfc-ndef-protocols; chip/key ->
  contactless-smartcards-mifare-desfire; attacks ->
  rfid-nfc-access-attacks-defenses; Wiegand/OSDP wire ->
  access-credentials-wiegand-osdp; panels/door-hardware/topology ->
  pacs-access-control-architecture; mobile/Aliro/UWB/mDL ->
  mobile-credentials-aliro-uwb.
version: 1.0.0
updated: 2026-06-18
metadata:
  changelog:
    - "2026-06-18 greenfield build (standards/convergence spoke of the physical-access family). Core: physical-logical convergence (PIAM, CSO/CISO, one credential); US federal identity (HSPD-12 -> FIPS 201-3/PIV, PIV-I, CAC, FICAM, OMB M-19-17, SP 800-116 r1 facility-area -> auth-factor model, GSA APL, Federal PKI/OCSP, SP 800-73/800-78 by name); standards/frameworks crosswalk table (SIA, ONVIF A/C/D, ISO/IEC 27001 A.7, SOC 2 CC6, PCI DSS req 9, ISO/IEC 60839); PSIM; visitor management & identity proofing; biometric-privacy (Illinois BIPA + 2024 SB 2979) and surveillance-notice, both flagged jurisdiction/date-dependent and not legal advice. references/federal-identity-piv-ficam.md and references/standards-frameworks-and-privacy.md."
---
