...
category: developer
whenToUse: >-
  Use when the task involves the PostgreSQL engine and data plane — diagnosing or tuning MVCC, VACUUM/autovacuum, bloat, or transaction-ID wraparound; reading EXPLAIN/EXPLAIN ANALYZE output and reasoning about the planner, cost model, and statistics; designing indexes or choosing an access method (B-tree, GIN, GiST, SP-GiST, BRIN, Hash); configuring WAL, checkpoints, PITR, streaming or logical replication, replication slots, and failover; declarative partitioning and large-table maintenance; modeling with JSONB, arrays, ranges, or extensions (PostGIS, pg_stat_statements, postgres_fdw, TimescaleDB, Citus, pgvector); reasoning about isolation levels, locking, and deadlocks; or writing advanced SQL (CTEs, window functions, MERGE, RLS). Not for connection poolers or query middleware (database-proxies-query-middleware), the AWS Aurora PostgreSQL platform layer (aws-cloud), MongoDB, or warehouse/dbt modeling.
keywords:
  - PostgreSQL
  - Postgres
  - MVCC
