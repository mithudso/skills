...
Treating documentation like source code — **markdown files in git, built into a site by a static-site
generator (SSG), reviewed via PR, deployed by CI**. This reference owns the docs-as-code workflow and
SSG selection. For markdown syntax load `references/markdown-authoring.md`; for MDX specifics load
`references/mdx.md`; for prose/structure quality defer to the `technical-writing-craft` hub (Diátaxis,
heading discipline) — this reference is about the *plumbing*, not the writing.

---

## 1. The docs-as-code workflow

1. **Author** markdown (+ frontmatter) alongside or near the code.
2. **Review** via pull request (diffs, comments, approvals — same as code).
3. **Build** with an SSG (markdown → static HTML/CSS/JS).
...
Benefits: versioned with the product, single source of truth, contributor-friendly, preview deploys per PR.
The content layer is almost always **markdown/MDX + frontmatter**; the SSG supplies nav, search,
versioning, theming.
...
| **MkDocs + Material** | Python | Fast, beautiful, low-friction docs | Material theme redefined the category; rich extensions (admonitions, tabs); great search |
| **VitePress** | Vue / Vite | JS-camp docs, fast dev | MkDocs-Material-like for the Vite world; Vue components in markdown |
...
**Selection heuristic:** React shop → Docusaurus or Nextra. Want best-in-class OSS docs with little
config → Starlight or MkDocs Material. Vue/Vite → VitePress. Massive site, speed paramount → Hugo.
Python/API docs from docstrings → Sphinx (+MyST for markdown). Hosted, zero-ops → Mintlify.

## 3. The content layer details

- **Frontmatter** drives per-page metadata (title, sidebar order, slug, tags, draft) — YAML is near-universal;
  Hugo also takes TOML/JSON. See `references/markdown-authoring.md` §4.
- **MDX** (Docusaurus, Astro, Nextra, VitePress-via-Vue) lets docs embed live components — see `references/mdx.md`.
- **Cross-references & anchors** — SSGs slugify headings; prefer relative links to source `.md` files
  (many SSGs rewrite them) so links survive both in-repo and on the built site.
- **Admonitions/callouts** — most doc SSGs support `:::note` / `> [!NOTE]` style callouts (syntax varies;
  Material uses `!!! note`, Docusaurus/Starlight use `:::note`).
...
Per-PR **preview deploys** (Netlify/Vercel/Cloudflare Pages) let reviewers see rendered docs before merge —
the highest-leverage docs-as-code practice.
