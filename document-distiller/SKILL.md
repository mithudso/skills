...
### Phase 1: Ingest
- Resolve the input:
  - **URL** → fetch page text using `web-text-mirror` pulling a depth of 1 at each stage. Fall back to built-in `WebFetch` if needed. Record the URL and fetch date.
...
- Write two files to `~/.research/distillations/` (create the dir if missing):
...
1. **Locate the existing distillation.** Resolve target by slug or path the user gives; if
   ambiguous, use the most recently generated `<slug>-*.json` in `~/.research/distillations/`
