...
mongodb:
  ...
  allow_large_context_reason: >
    The skill bundles the ftdc_parser Go module (src/) that compiles to the
    tool the skill drives. That source is a build input, not agent context —
    the agent runs the binary, it never loads the .go files. SKILL.md and
