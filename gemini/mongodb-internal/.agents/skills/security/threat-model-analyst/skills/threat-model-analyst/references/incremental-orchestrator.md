...
3. **Validate inputs and detect baseline type:**
   - Confirm baseline folder exists: `Test-Path {baseline_folder}/threat-inventory.json`
   - Read `source_documents` from baseline `threat-inventory.json`
   - Read baseline `commit` field from `threat-inventory.json`

   **Determine `BASELINE_TYPE`:**
   - If baseline `commit` is `"N/A — document analysis"` → `BASELINE_TYPE = "document"`
   - Otherwise → `BASELINE_TYPE = "repository"`

   **Determine `TARGET_TYPE`:**
   - If the user's request references a git repo or specific commit → `TARGET_TYPE = "repository"`
   - If the user's request references a document URL or Glean doc → `TARGET_TYPE = "document"`

   **Set `TRANSITION_MODE`:**
   - `BASELINE_TYPE = "document"` AND `TARGET_TYPE = "repository"` → `TRANSITION_MODE = true` (design doc → code implementation)
   - `BASELINE_TYPE = "repository"` AND `TARGET_TYPE = "document"` → unsupported; surface an error and ask the user to clarify
   - All other combinations → `TRANSITION_MODE = false`; proceed with standard incremental workflow

   **If `TRANSITION_MODE = false` (standard repo-to-repo run):**
   ...
   **If `TRANSITION_MODE = true` (doc → code):**
   - Set `baseline_sha = "N/A — document analysis"` (canonical sentinel — matches the value set by `orchestrator.md` for document-based runs)
   - Resolve `target_sha`: use the user-provided target ref if present (`git rev-parse --short {user_ref}`), otherwise default to `git rev-parse --short HEAD`
   - Set `COMMIT_COUNT = "N/A"` and `PR_COUNT = "N/A"` (no meaningful commit range)
   - Skip git commit date lookups for the baseline; still get target commit date: `git log -1 --format="%ai" {target_sha}`

4. **Re-pull source documents (if applicable):**
   Read `source_documents` from baseline `threat-inventory.json`.

   - If `source_documents` is non-empty:
     - For each entry where `retrieval_tool` is `glean`: re-fetch the document via Glean MCP using the stored `url`
     - For each entry where `retrieval_tool` is `web`: re-fetch via URL
     - Store the refreshed document content for use in Phase 2 change detection (comparing against the document version used in the baseline run)
     - Record the new `retrieved_at` timestamp (use `START_TIME`) in the new report's `source_documents`
     - If a document cannot be fetched (access denied, document deleted, etc.): log in `### Needs Verification` with the stored `url`, `title`, and failure reason. Continue analysis using only available documents.
   - If `source_documents` is empty: this is a repository-based incremental run — skip this step and proceed with git-based change detection.

5. **Baseline code access — reuse or create worktree (`TRANSITION_MODE = false` only):**

   **Skip this step entirely if `TRANSITION_MODE = true`.** There is no baseline code to compare against — the baseline was a document. Set `BASELINE_WORKTREE = null`.

   If `TRANSITION_MODE = false`:
   ```
...
6. **Create output folder:**
   ```
   threat-model-{TICKET-ID}/        ← preferred (e.g., threat-model-CLOUDP-353786)
   threat-model-{YYYYMMDD-HHmmss}/  ← fallback if no ticket ID in prompt
   ```
   Extract the ticket ID (matching `CLOUDP-\d+` or `SERVER-\d+`) from the user's prompt. Use it if present; otherwise fall back to timestamp.
...
**If `TRANSITION_MODE = true` (doc → code), use the [Transition Mode path](#phase-2-transition-mode-doc--code) below instead of the standard git-diff path.**

### Phase 2: Standard Mode (repo → repo)
...
### Phase 2: Transition Mode (doc → code)

There is no baseline code to diff against. Instead, match document-described components against the actual codebase:

```
For EACH component in inherited inventory (derived from design document):

  1. Search the codebase for a corresponding implementation:
     - Match by component name, role, described responsibilities, and data flows
     - Use the same component discovery rules from orchestrator.md (class names,
       manifests, config files)
     - Apply the COMPONENT ANCHOR VERIFICATION rule — every match must cite a
       real file path

  2. If a matching implementation is found:
     → change_status = "implementation_review"
     → Record the source_files from the codebase
     → STRIDE analysis will verify design assumptions against actual code

  3. If NO matching implementation is found:
     → change_status = "not_implemented"
     → Mark all linked threats as "not_implemented"
     → Mark all linked findings as "not_implemented"
     → Add to Needs Verification: note that this component was described in the
       design document but has no identifiable implementation in the codebase
```

**`implementation_review` STRIDE approach:** For each `implementation_review` component, perform a fresh STRIDE analysis of the actual code, then cross-reference against the design-based threats from the baseline:
- Design threat confirmed by code → `change_status = "confirmed_in_implementation"`
- Design threat NOT found in code (mitigated by implementation) → `change_status = "fixed"` (cite the implementation pattern that addresses it)
- New threat found in code that was not in the design → `change_status = "new_in_implementation"`

**Display tags for transition mode** (body text, not headings):
- `> **[Confirmed]**` — design threat verified in implementation
- `> **[Fixed]**` — design threat not present in implementation (cite evidence)
- `> **[New]**` — threat found in code, not in design document
- `> **[Not Implemented]**` — component or threat has no corresponding code yet
...
**⛔ HEADING ANCHOR RULE (applies to ALL output files):** ALL `##` and `###` headings in every output file must be PLAIN text — NO status tags (`[Existing]`, `[Fixed]`, `[Partial]`, `[New]`, `[Removed]`, `[Confirmed]`, `[Not Implemented]`, or any old-style tags) in heading text. Tags break markdown anchor links and pollute table-of-contents. Place status annotations on the FIRST LINE of the section/finding body instead:
- ✅ `## KmsPluginProvider` with first line `> **[New]** Component added in this release.`
- ✅ `### FIND-01: Missing Auth Check` with first line `> **[Existing]**` (standard) or `> **[Confirmed]**` (transition)
- ❌ `## KmsPluginProvider [New]` (breaks `#kmspluginprovider` anchor)
- ❌ `### FIND-01: Missing Auth Check [Confirmed]` (pollutes heading)
...
| **Implementation Review** *(transition mode only)* | Fresh STRIDE-A analysis of the actual code. Cross-reference each threat against the design-based baseline: confirmed → `confirmed_in_implementation`; mitigated by implementation → `fixed`; new in code → `new_in_implementation`. |
| **Not Implemented** *(transition mode only)* | Section header with note: "Component described in design document but not found in codebase — all threats marked `not_implemented`." |
...
<!-- SIMPLIFIED DISPLAY TAGS: Only 5 tags for display in markdown body text.
  ...
  Transition mode (doc → code):
  [Confirmed] = confirmed_in_implementation (design threat verified in code)
  [Fixed] = fixed (design threat not present — implementation addressed it)
  [New] = new_in_implementation (found in code, not in design)
  [Not Implemented] = not_implemented (component/threat has no code yet)
...
**Additional finding situations for transition mode (`TRANSITION_MODE = true`):**

| Situation | change_status | Label |
|-----------|---------------|-------|
| Design finding confirmed by code evidence | `confirmed_in_implementation` | `> **[Confirmed]**` — cite the code pattern that confirms it |
| Design finding not present in implementation | `fixed` | `> **[Fixed]**` — cite the implementation pattern that addresses it |
| New finding discovered in code, not in design | `new_in_implementation` | `> **[New]**` — note it was not described in the design document |
| Finding for a component with no code yet | `not_implemented` | `> **[Not Implemented]**` — note component is pending implementation |

<!-- ⛔ POST-STEP CHECK: After writing all finding annotations:
  Standard mode — every finding body starts with one of: [Existing], [Fixed], [Partial], [New], [Removed]
  Transition mode — every finding body starts with one of: [Confirmed], [Fixed], [New], [Not Implemented]
  ...
  4. JSON change_status uses the detailed values (still_present, confirmed_in_implementation, new_in_implementation, etc.) for programmatic comparison -->
...
### Rule I9: Transition Mode (Doc → Code)

When `TRANSITION_MODE = true`, the following rules apply in addition to I1–I8:

- **Skip git worktree creation** (Phase 0, Step 5) — there is no baseline code.
- **Skip git-diff change detection** (Phase 2 standard path) — use the transition mode path instead.
- **`confirmed_in_implementation` requires code evidence** — cite the specific file, class, or config that confirms the design threat exists in the implementation. No guessing.
- **`fixed` in transition mode requires implementation evidence** — cite the specific pattern (e.g., a validation function, an auth check, a config value) that addresses the design threat. This is different from standard `fixed`, which requires a code diff.
- **`not_implemented` components go to Needs Verification** — the engineer must confirm whether the component is genuinely not yet built, or was renamed/restructured in a way the analysis missed.
- **Rule I6 does not apply** — there is no baseline worktree to preserve.
- **Report Metadata additions for transition mode:**
  ```
  | Analysis Mode | Transition (Document → Code) |
  | Baseline Document | {title} — {url} |
  | Baseline Report | {baseline_folder} |
...
|-------|--------|-----------------|
| 0 | Setup, detect baseline/target types, set TRANSITION_MODE, re-pull source documents, worktree (if applicable) | TRANSITION_MODE set, source documents fetched or flagged, worktree accessible (or skipped in transition mode) |
| 1 | Load old inventory skeleton | All arrays populated, metrics match |
| 2 | Per-component change detection (standard) or implementation matching (transition mode) | Every component has a `change_status` |
