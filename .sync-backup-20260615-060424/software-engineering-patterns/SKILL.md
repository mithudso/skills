---
description: >-
  Language-agnostic software design, architecture & engineering-practice hub. TRIGGER: API design (REST/GraphQL/gRPC, versioning); backend & microservices patterns (decomposition, bounded contexts, strangler fig); Express/Fastify/NestJS/Hono framework selection; Python web frameworks (FastAPI/Django/Flask, ASGI/WSGI, uvicorn/gunicorn); design patterns (creational/structural/behavioral); coding standards; development antipatterns & code smells; architecture methodology & docs (ISO 42010, ADRs, C4); code review (OWASP checklist); debugging & root-cause (5 Whys); performance profiling; SSE/streaming; job scheduling / MV3 alarms; auth-checker/web-auth; diagnostic registry; Vitest testing (vi mocks, coverage, snapshots, Jest→Vitest); Playwright web-app/UI testing; repo reusable-pattern scanning. SKIP: language syntax/idioms → lang-python/lang-js-ts/lang-go-and-mobile; frontend/UI/CSS → frontend-ui; SaaS API clients (Jira/Slack/Salesforce/Monday) → integration-clients.
...
tags:
  ...
  - code-review
whenToUse:
  - "designing an API surface (REST/GraphQL/gRPC), resource model, or versioning strategy"
  - "choosing backend or microservices patterns — decomposition, boundaries, Express middleware"
  - "picking or building a Node.js backend framework — Fastify plugins/hooks, NestJS modules/DI/guards, or Hono on edge runtimes"
  - "picking or building a Python web framework — FastAPI, Django, or Flask, including async-vs-sync routes and ASGI/WSGI server setup"
  - "applying a design pattern or enforcing coding standards across a codebase"
  - "doing software-architecture work: tradeoffs, ADRs, C4/42010 documentation"
  - "reviewing code or a PR for correctness, security (OWASP), and maintainability"
  - "debugging methodically or profiling a performance problem"
  - "implementing SSE/streaming, job/alarm scheduling, or registry/dispatch patterns"
  - "wiring auth checks, template/config, IndexedDB, or integration-client scaffolding"
whenNotToUse:
  - "Language syntax or idioms (Python, Go, TS, JS/Node, Kotlin) — use programming-languages"
  - "Frontend, UI, or CSS/component design — use frontend-ui"
  - "AI/agent/LLM system or prompt design — use ai-agent-engineering"
  - "Infra, CI/CD, containers, or observability — use devops-infra"
  - "Vendor API client specifics (endpoints, SDK quirks) — use integration-clients"
related_skills:
